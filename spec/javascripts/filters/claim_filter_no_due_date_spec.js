...
describe('claimFilterNoDueDate Filter', function() {
  ...
  it('has a claimFilterNoDueDate filter', function () {
      expect($filter('claimFilterNoDueDate')).toBeDefined();
  });

  it('should filter claims by no due date', function () {
    var filtered = $filter('claimFilterNoDueDate')(claims);
    expect(_.pluck(filtered, 'claim_id')).toEqual([ 1, 7, 8 ]);
  });
});
